...
**The easy way.** Open the Arc Names app and press **Connect**. If your wallet is on a different
network, a banner appears at the top with a **Switch network** button. Most wallets will offer to
add Arc Testnet automatically when you press it.
...
you've set one). If something is wrong, a banner appears under the header:
...
Press your address in the header and choose disconnect. This only ends the app's session — it does
not move, lock, or affect your names in any way. Your names live in the wallet, not in the app.
