...
function assert4(config) {
  ...
  for (const owner of owners) {
    if (!validate2(owner.owner) || toBigInt(owner.owner) === 0n)
...
function assertMultisig(envelope, depth) {
  ...
  if (!validate2(envelope.account))
    throw new InvalidMultisigApprovalError({
      reason: "multisig account is invalid"
    });
  if (toBigInt(envelope.account) === 0n)
    throw new InvalidMultisigApprovalError({
...
function deserialize_(value, multisigDepth) {
  ...
  if (typeId === serializedKeychainType || typeId === serializedKeychainV2Type) {
    ...
    const inner = deserialize_(slice(data, 20), multisigDepth);
    return {
      userAddress,
      inner,
      type: "keychain",
  ...
  if (typeId === serializedMultisigType) {
    ...
    if (!Array.isArray(address2) && !validate2(address2))
      throw new InvalidSerializedError2({
...
function fromRpc5(envelope) {
  ...
  if (envelope.type === "keychain" || "userAddress" in envelope && "signature" in envelope) {
    ...
    return {
      ...
      inner: fromRpc5(keychain.signature),
  ...
  if (envelope.type === "multisig" || "signatures" in envelope && ("account" in envelope || "init" in envelope)) {
    ...
    const init = hasInit ? from6(multisig.init) : void 0;
    const account = init ? getAddress(init) : multisig.account;
    const result = {
      type: "multisig",
      account,
      signatures: multisig.signatures.map((signature) => fromRpc5(signature)),
      ...init ? { init } : {}
    };
    assert5(result);
    return result;
...
function toRpc5(envelope) {
  ...
  if (type2 === "multisig") {
    const multisig = envelope;
    assert5(multisig);
    const signatures = multisig.signatures.map((signature) => toRpc5(signature));
    if (multisig.init) {
      const init = {
        ...multisig.init,
...
function from11(authorization, options = {}) {
  ...
  if (options.signature) {
    const signature = from7(options.signature);
...
var transferBlocked = from5("event TransferBlocked(address indexed token, address indexed receiver, uint64 indexed blockedNonce, uint256 amount, uint8 receiptVersion, bytes receipt)");
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload4(from14(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload4(from14(envelope)),
...
//# sourceMappingURL=chunk-DNYYWI3X.js.map
