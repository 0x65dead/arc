...
function toString4(value) {
  return toString(toBytes6(value));
}
...
function encode4(hrp, data, options = {}) {
...
function createChecksum(hrp, data) {
  ...
  const mod3 = polymod(values) ^ BECH32M_CONST;
  const ret = [];
  for (let i = 0; i < 6; i++)
    ret.push(mod3 >> 5 * (5 - i) & 31);
  return ret;
}
...
function fromHex6(hex2, group) {
  return fromBytes6(toBytes(hex2), group);
}
...
(function(decodeCursor2) {
  ...
  function getFloat16(bits) {
    ...
    if (exponent === 0) {
      ...
      return sign7 ? -value2 : value2;
    }
    if (exponent === 31) {
      if (fraction === 0)
        return sign7 ? -Infinity : Infinity;
      return NaN;
    }
    const value = 2 ** (exponent - 15) * (1 + fraction / 1024);
    return sign7 ? -value : value;
  }
...
function toHex9(value) {
  return fromBytes(toBytes9(value));
}
function fromBytes7(data) {
  if (data.length === 0)
...
function fromHex7(data) {
  return fromBytes7(fromHex(data));
}
...
function fromPublicKey(publicKey) {
  ...
  return encode5(/* @__PURE__ */ new Map([
    [1, 2],
    // kty: EC2
    [3, -7],
    // alg: ES256
...
function createCurve2(curveDef, defHash) {
  const create5 = (hash7) => weierstrass({ ...curveDef, hash: hash7 });
...
function chain(...args) {
  ...
  return { encode: encode9, decode: decode6 };
}
...
function genBech32(encoding) {
  ...
  function decode6(str, limit2 = 90) {
...
var hex = hasHexBuiltin2 ? hexBuiltin : chain(radix2(4), alphabet3("0123456789abcdef"), join(""), normalize((s) => {
...
function wrapConstructorWithKey(hashCons) {
  const hashC = (msg, key) => hashCons(key, msg.length).update(toBytes11(msg)).digest();
...
function verify3(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex2(publicKey).substring(2), { lowS: true, ...hash7 ? { prehash: true } : {} });
}
...
async function create4(options) {
...
function getClientDataJSON(options) {
  const { challenge: challenge2, crossOrigin = false, extraClientData, origin = window.location.origin, type: type6 = "webauthn.get" } = options;
...
function verify4(options) {
  const { challenge: challenge2, metadata, origin, publicKey, rpId, signature } = options;
  ...
  if (clientData.type !== "webauthn.get")
    return false;
  if (!clientData.challenge || fromBytes(toBytes6(clientData.challenge)) !== challenge2)
    return false;
  if (origin !== void 0) {
...
export {
  ...
  fromRpc2 as fromRpc,
  toRpc3 as toRpc,
  getPublicKey4 as getPublicKey,
  randomPrivateKey3 as randomPrivateKey,
  sign3 as sign,
  verify3 as verify,
  DisconnectedError,
  ...
  TipAboveFeeCapError,
  createCredential,
  getSignPayload10 as getSignPayload,
  sign5 as sign2,
  verify5 as verify2,
  createKeyPair4 as createKeyPair,
  sign6 as sign3,
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-ACH3YCGD.js.map
