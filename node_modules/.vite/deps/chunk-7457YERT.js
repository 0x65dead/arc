...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-XDACXEPI.js");
  const signature_ = (() => {
...
async function prepareTransactionRequest(client, args) {
  ...
  const fillResult = attemptFill ? await getAction(client, fillTransaction, "fillTransaction")({ ...request, nonce }).then((result) => {
    const { chainId: chainId2, from: from9, gas: gas2, gasPrice, nonce: nonce2, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, type: type2, ...rest } = result.transaction;
...
var suffixParameters = from2("(uint256 chainId, address delegation, uint256 nonce, uint8 yParity, uint256 r, uint256 s), address to, bytes data");
...
function wrap(value) {
  ...
  const self = recoverAddress({
    ...
    signature: from3(value.authorization)
  });
...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account ? encode2(from5("constructor(bytes, bytes)"), {
    bytecode: deploylessCallViaBytecodeBytecode,
    args: [
      getBalanceCode,
      encodeData(from6("function getBalance(address)"), [account.address])
    ]
  }) : void 0;
      ...
      ...traceAssetChanges ? [
        ...
        // Asset pre balances
        {
          calls: assetAddresses.map((address, i) => ({
            abi: [
              from6("function balanceOf(address) returns (uint256)")
            ],
      ...
      ...traceAssetChanges ? [
        ...
        // Decimals
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [
              from6("function decimals() returns (uint256)")
            ],
        ...
        // Token URI
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [
              from6("function tokenURI(uint256) returns (string)")
            ],
        ...
        // Symbols
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [from6("function symbol() returns (string)")],
            functionName: "symbol",
            from: zeroAddress,
            nonce: i
          })),
...
function unwrap2(wrapped) {
  assert3(wrapped);
  const [to, data, signature] = decode(from2("address, bytes, bytes"), wrapped);
  return { data, signature, to };
}
function wrap2(value) {
  const { data, signature, to } = value;
  return concat2(encode(from2("address, bytes, bytes"), [
    to,
    data,
    signature
  ]), magicBytes2);
...
function getCall2(client, parameters) {
  const { amount, from: from9, to, token } = parameters;
  const { address, decimals } = resolveToken(client, { token });
...
//# sourceMappingURL=chunk-7457YERT.js.map
