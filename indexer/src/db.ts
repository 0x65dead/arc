...
export async function getSyncBlock(stream: string, fallback: number): Promise<number> {
...
export async function setSyncBlock(stream: string, block: number): Promise<void> {
  await pool.query(
    `INSERT INTO sync_state (stream, last_block, updated_at)
     VALUES ($1, $2, now())
     ON CONFLICT (stream) DO UPDATE SET last_block = $2, updated_at = now()`,
    [stream, block]
  );
}
...
export async function upsertRegistration(row: {
  ...
  await pool.query(
    `INSERT INTO registrations (name, token_id, owner, cost_wei, expires_at, block_number, tx_hash)
    ...
    [row.name, row.tokenId, row.owner, row.costWei, row.expiresAt, row.blockNumber, row.txHash]
  );
}
...
export async function insertRenewal(row: {
  ...
  await pool.query(
    `INSERT INTO renewals (name, cost_wei, expires_at, block_number, tx_hash, log_index)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (tx_hash, log_index) DO NOTHING`,
    [row.name, row.costWei, row.expiresAt, row.blockNumber, row.txHash, row.logIndex]
  );
  await pool.query(
    `UPDATE registrations SET expires_at = GREATEST(expires_at, $2) WHERE name = $1`,
    [row.name, row.expiresAt]
  );
}
...
export async function insertTransfer(row: {
  ...
  await pool.query(
    `INSERT INTO transfers (token_id, from_addr, to_addr, block_number, tx_hash, log_index)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (tx_hash, log_index) DO NOTHING`,
    [row.tokenId, row.fromAddr, row.toAddr, row.blockNumber, row.txHash, row.logIndex]
  );
  ...
  await pool.query(
    `UPDATE registrations SET owner = $2 WHERE token_id = $1`,
    [row.tokenId, row.toAddr]
  );
}
...
export async function upsertListing(row: {
  ...
  await pool.query(
    `INSERT INTO listings (token_id, seller, price_wei, status, block_number, tx_hash, updated_at)
     ...
     ON CONFLICT (token_id) DO UPDATE SET
       ...
       tx_hash = EXCLUDED.tx_hash,
       updated_at = now()`,
    [row.tokenId, row.seller, row.priceWei, row.blockNumber, row.txHash]
  );
}

export async function markListingStatus(tokenId: string, status: 'sold' | 'cancelled'): Promise<void> {
  await pool.query(
    `UPDATE listings SET status = $2, updated_at = now() WHERE token_id = $1`,
    [tokenId, status]
  );
}
...
export async function insertSale(row: {
  ...
  await pool.query(
    `INSERT INTO sales (token_id, seller, buyer, price_wei, fee_wei, block_number, tx_hash, log_index)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     ON CONFLICT (tx_hash, log_index) DO NOTHING`,
    [row.tokenId, row.seller, row.buyer, row.priceWei, row.feeWei, row.blockNumber, row.txHash, row.logIndex]
  );
}
...
export async function getAvailability(name: string): Promise<{
  taken: boolean;
  owner?: string;
  expiresAt?: number;
}> {
...
export async function getDomainsByOwner(owner: string) {
  const { rows } = await pool.query(
    `SELECT name, token_id, owner, expires_at, block_number, tx_hash
     FROM registrations
...
export async function getActiveListings() {
  const { rows } = await pool.query(
    `SELECT l.token_id, r.name AS name, l.seller, l.price_wei, l.updated_at
     FROM listings l
