...
function toNormalizedLog(item: BlockscoutLogItem): NormalizedLog {
  return {
    ...
    topics: item.topics.filter((t): t is string => t !== null),
    data: item.data,
...
export async function fetchAllLogsForAddress(address: string): Promise<NormalizedLog[]> {
  ...
  while (true) {
    ...
    page++;
    if (!body.next_page_params || page >= MAX_PAGES) break;
