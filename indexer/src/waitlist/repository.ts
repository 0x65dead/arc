...
export async function createNonce(address: string, message: (nonce: string, issuedAt: Date, expiresAt: Date) => string): Promise<Challenge> {
  ...
  await pool.query(
    `INSERT INTO waitlist_nonces (nonce, address, message, issued_at, expires_at)
     VALUES ($1, $2, $3, $4, $5)`,
    [nonce, address.toLowerCase(), text, issuedAt, expiresAt],
  );

  return { nonce, message: text, expiresAt };
}

export type NonceLookup =
  | { ok: true; message: string }
  | { ok: false; reason: 'unknown' | 'expired' | 'consumed' | 'address-mismatch' };
 ...
 */
export async function claimNonce(nonce: string, address: string): Promise<NonceLookup> {
  const claimed = await pool.query<{ message: string }>(
    `UPDATE waitlist_nonces
        SET consumed_at = now()
      WHERE nonce = $1
        AND address = $2
  ...
  if (claimed.rowCount) return { ok: true, message: claimed.rows[0].message };

  const existing = await pool.query<{ address: string; consumed_at: Date | null; expires_at: Date }>(
    `SELECT address, consumed_at, expires_at FROM waitlist_nonces WHERE nonce = $1`,
    [nonce],
  );

  if (!existing.rowCount) return { ok: false, reason: 'unknown' };
  const row = existing.rows[0];
  if (row.address !== address.toLowerCase()) return { ok: false, reason: 'address-mismatch' };
...
export async function createSession(address: string): Promise<{ token: string; expiresAt: Date }> {
  const token = newToken();
  const expiresAt = new Date(Date.now() + config.waitlist.sessionTtlMs);

  await pool.query(
    `INSERT INTO waitlist_sessions (token_hash, address, expires_at) VALUES ($1, $2, $3)`,
    [hashToken(token), address.toLowerCase(), expiresAt],
  );

  return { token, expiresAt };
}

export async function findSession(token: string): Promise<{ address: string } | null> {
  const { rows } = await pool.query<{ address: string }>(
    `SELECT address FROM waitlist_sessions WHERE token_hash = $1 AND expires_at > now()`,
    [hashToken(token)],
  );
  return rows[0] ?? null;
}
 ...
 */
export async function issueOAuthState(token: string): Promise<string | null> {
  const state = newToken();
  const { rowCount } = await pool.query(
    `UPDATE waitlist_sessions
        SET oauth_state = $2, oauth_state_at = now()
      WHERE token_hash = $1 AND expires_at > now()`,
    [hashToken(token), hashToken(state)],
  );
 ...
 */
export async function consumeOAuthState(state: string): Promise<{ address: string } | null> {
  const { rows } = await pool.query<{ address: string }>(
    `UPDATE waitlist_sessions
...
export interface WaitlistEntry {
  ...
  roleGranted: boolean;
  createdAt: string;
  /** 1-based position among verified entries, or null until verified. */
...
export async function getEntry(address: string): Promise<WaitlistEntry | null> {
  const { rows } = await pool.query<EntryRow>(`${ENTRY_SELECT} WHERE e.address = $1`, [
    address.toLowerCase(),
  ]);
...
export async function upsertSigned(address: string): Promise<void> {
  await pool.query(
    `INSERT INTO waitlist_entries (address, signed_at)
     VALUES ($1, now())
     ON CONFLICT (address) DO UPDATE SET signed_at = now(), updated_at = now()`,
    [address.toLowerCase()],
  );
}

export class DiscordAlreadyLinkedError extends Error {
  constructor(readonly linkedAddress: string) {
...
export async function linkDiscord(params: {
  ...
  try {
    ...
    if ((error as { code?: string }).code === '23505') {
      const { rows } = await pool.query<{ address: string }>(
        `SELECT address FROM waitlist_entries WHERE discord_id = $1`,
        [params.discordId],
      );
  ...
  if (!entry) throw new Error('waitlist entry vanished immediately after write');
  return entry;
}

export async function countVerified(): Promise<number> {
  const { rows } = await pool.query<{ count: string }>(
    `SELECT COUNT(*) AS count FROM waitlist_entries WHERE verified`,
  );
