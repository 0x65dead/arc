...
function toNormalizedLog(address: string, item: BlockscoutLogItem): NormalizedLog {
  return {
    ...
    // real hex topics.
    topics: item.topics.filter((topic): topic is string => topic !== null),
    data: item.data,
...
async function fetchJson(url: string): Promise<BlockscoutLogsResponse> {
  ...
  try {
    ...
    return (await response.json()) as BlockscoutLogsResponse;
  } catch (error) {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      throw new Error(
        `Blockscout timed out after ${config.blockscoutTimeoutMs}ms (BLOCKSCOUT_TIMEOUT_MS): ${url}`,
      );
    }
    throw error;
  } finally {
...
export async function fetchLogsSince(
  ...
  while (page < MAX_PAGES) {
    ...
    for (const item of items) {
      ...
      if (item.block_number > toBlock) continue;
