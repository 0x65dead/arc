...
async function request(path, options) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
            ...
            throw new DiscordUnavailableError(controller.signal.aborted ? `Discord timed out after ${TIMEOUT_MS}ms` : detail);
        }
        finally {
            clearTimeout(timer);
        }
        ...
        if (!response.ok) {
            ...
            throw new DiscordApiError(response.status, body?.code ?? null, body?.error_description ?? body?.message ?? `Discord returned HTTP ${response.status}`);
        }
...
export async function fetchUser(accessToken) {
    const user = await request('/users/@me', { auth: `Bearer ${accessToken}` });
    if (!user?.id)
        throw new DiscordApiError(502, null, 'Discord returned no user');
    return {
        id: user.id,
...
export async function isGuildMember(discordId) {
    ...
    const member = await request(`/guilds/${guildId}/members/${discordId}`, { auth: botAuth(), nullOn: [404] });
    return member !== null;
}
/**
 ...
 */
export async function addGuildMember(discordId, accessToken) {
    const { guildId } = config.waitlist.discord;
    try {
 ...
 * instead of throwing.
 */
export async function grantWaitlistRole(discordId) {
 ...
 */
export function notifySignup(entry) {
