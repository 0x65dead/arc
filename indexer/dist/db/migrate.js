...
async function ensureLedger() {
    ...
    `);
        ...
        if (error.code === '42501') {
            throw new Error('permission denied creating tables: the DATABASE_URL role cannot CREATE in its schema.\n' +
...
export async function runMigrations() {
    ...
    try {
        ...
        throw new Error(`Cannot read migrations directory ${MIGRATIONS_DIR}: ${error.message}`);
    }
    if (files.length === 0)
