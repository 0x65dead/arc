...
import { getSyncBlock, insertRenewal, insertSale, insertTransfer, markListingStatus, setSyncBlock, upsertListing, upsertRegistration } from './db.js';
import { fetchAllLogsForAddress } from './blockscout.js';
...
// ================================================================
function sortChronological(logs) {
...
async function processControllerLog(log) {
...
async function processRegistrarLog(log) {
...
async function processMarketLog(log) {
...
async function runBackfillFromArcScan() {
    ...
    for (const log of sortChronological(controllerLogs))
        await processControllerLog(log);
    for (const log of sortChronological(registrarLogs))
        await processRegistrarLog(log);
    for (const log of sortChronological(marketLogs))
        await processMarketLog(log);
    console.log(`[backfill:arcscan] processed ${controllerLogs.length} controller / ` +
...
async function getLogsWithRetry(provider, filter, fromBlock, toBlockRequested, label, maxRetries = 5) {
    let attempt = 0;
        ...
        try {
            ...
            if (suggested !== null) {
                ...
                if (newSize < chunkSizeRef.value) {
                    console.warn(`[backfill:rpc:${label}] provider enforces a ${newSize}-block eth_getLogs range cap — ` +
...
async function scanStream(provider, stream, address, topics, latestBlock, onLogs) {
...
async function runBackfillFromRpc(provider) {
...
// Entry point
// ================================================================
export async function runBackfill(provider) {
    try {
