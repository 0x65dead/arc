import { CONTROLLER_EVENTS, MARKET_EVENTS, REGISTRAR_EVENTS, TOPICS, labelToId, normalizeLabel, toFullName, } from '../chain/abis.js';
import { getBlockTime, readListingCurrency } from '../chain/provider.js';
import { withTransaction } from '../db/pool.js';
import { closeListing, insertEvent, insertRenewal, insertSale, insertTransfer, upsertListing, upsertRegistration, } from '../db/repositories.js';
...
export function sortChronological(logs) {
    return [...logs].sort((a, b) => a.blockNumber - b.blockNumber || a.logIndex - b.logIndex);
}
export function toNormalized(log) {
    return {
...
async function handleController(entry, blockTime) {
...
async function handleRegistrar(entry, blockTime) {
    if (entry.topics[0] !== TOPICS.Transfer)
        return;
...
async function handleMarket(entry, blockTime) {
 ...
 */
export async function processLog(entry) {
    const address = entry.address.toLowerCase();
    const blockTime = await getBlockTime(entry.blockNumber);
    if (address === config.contracts.controller)
        return handleController(entry, blockTime);
    if (address === config.contracts.registrar)
        return handleRegistrar(entry, blockTime);
    if (address === config.contracts.market)
        return handleMarket(entry, blockTime);
}
/**
