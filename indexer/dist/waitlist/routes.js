...
import { DiscordApiError, DiscordUnavailableError, addGuildMember, authorizeUrl, exchangeCode, fetchUser, grantWaitlistRole, isGuildMember, notifySignup, oauthScopes, } from '../discord/client.js';
import { buildSignInMessage, verifySignature } from './signature.js';
import { DiscordAlreadyLinkedError, claimNonce, consumeOAuthState, countVerified, createNonce, createSession, findSession, getEntry, issueOAuthState, linkDiscord, upsertSigned, } from './repository.js';
...
export function createWaitlistRouter() {
    ...
    router.use((req, _res, next) => {
        if (req.path === '/config' || isWaitlistEnabled())
            return next();
        next(new ApiError(503, 'unavailable', `The waitlist is not configured on this deployment (missing: ${missingWaitlistConfig().join(', ')})`));
    });
    ...
    // -------------------------------------------------------------------------
    router.post('/nonce', rateLimit({ max: 10, windowMs: 60_000 }), asyncHandler(async (req, res) => {
        const address = requireAddress(req.body?.address, 'address');
        const challenge = await createNonce(address, (nonce, issuedAt, expiresAt) => buildSignInMessage({
            address,
            nonce,
            domain: siteDomain(),
            issuedAt,
    ...
    router.post('/verify', rateLimit({ max: 10, windowMs: 60_000 }), asyncHandler(async (req, res) => {
        const address = requireAddress(req.body?.address, 'address');
    ...
    router.get('/discord', asyncHandler(async (req, res) => {
        ...
        if (!token)
            throw ApiError.badRequest('Sign with your wallet first.');
    ...
    router.get('/discord/callback', asyncHandler(async (req, res) => {
        ...
        if (!code || !state)
            return res.redirect(302, appRedirect({ status: 'invalid' }));
        ...
        try {
            ...
            return res.redirect(302, appRedirect({ status: member ? 'ok' : 'not-a-member' }));
        }
        catch (error) {
     ...
     */
    router.post('/recheck', rateLimit({ max: 6, windowMs: 60_000 }), asyncHandler(async (req, res) => {
...
async function requireSession(req) {
    ...
    if (!token)
        throw new ApiError(401, 'unauthorized', 'Sign with your wallet first.');
...
function rateLimit({ max, windowMs }) {
    ...
    return (req, _res, next) => {
        ...
        entry.count += 1;
        if (entry.count > max) {
            return next(new ApiError(429, 'rate_limited', 'Too many attempts. Wait a minute and try again.'));
        }
        next();
    };
}
