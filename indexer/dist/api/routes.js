...
import { getActivity, getAvailability, getDomain, getDomainsByOwner, getMarketplace, getStats, getSyncStreams, } from '../db/repositories.js';
import { getChainHead } from '../chain/provider.js';
import { config } from '../config.js';
import { createLogger } from '../logger.js';
import { ApiError, asyncHandler, optionalCursor, optionalLimit, optionalName, requireAddress, requireName, } from './errors.js';
...
export function createV1Router() {
    ...
    router.get('/domains/:name', asyncHandler(async (req, res) => {
        const name = requireName(req.params.name, 'name');
        const domain = await getDomain(name);
        if (!domain)
            throw ApiError.notFound(`${name} has never been registered`);
        res.json(domain);
    }));
    ...
    router.get('/sync-status', asyncHandler(async (_req, res) => {
        ...
        const checkpointAgeSeconds = streams.length
            ? Math.floor((Date.now() - Math.max(...streams.map((s) => Date.parse(s.updatedAt)))) / 1000)
            : null;
        const reconcileSeconds = config.reconcileIntervalMs / 1000;
        const checkpointFresh = checkpointAgeSeconds !== null && checkpointAgeSeconds <= reconcileSeconds * 3;
        res.json({
            indexedBlock,
            chainHead,
            blocksBehind,
            checkpointAgeSeconds,
            healthy: streams.length > 0 &&
                checkpointFresh &&
